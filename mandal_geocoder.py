...
def build_mandal_lookup(df: pd.DataFrame, save_path: str = LOOKUP_FILE) -> pd.DataFrame:
    ...
    if unmapped:
        ...
        master_csv = r"C:\Users\Eshwar Vijay Dheeraj\Downloads\ap_mandals_latlong.csv"
        
        if not os.path.exists(master_csv):
            print(f"  ERROR: Master CSV not found at {master_csv}. Cannot fill missing mandals.")
        else:
            print(f"  Attempting to fill from {master_csv}...")
            try:
                import difflib
                master_df = pd.read_csv(master_csv)
                # Standardize
                master_df["mandal_name"] = master_df["mandal_name"].astype(str).str.strip().str.title()
                if "district" in master_df.columns:
                    master_df["district"] = master_df["district"].astype(str).str.strip().str.title()
                    
                fallback_rows = []
                master_mandals = master_df["mandal_name"].dropna().unique().tolist()
                
                for m in sorted(unmapped):
                    districts_for_m = target[target[mandal_col] == m][district_col].dropna().unique()
                    d_name = districts_for_m[0] if len(districts_for_m) > 0 else "Unknown"
                    
                    # 1. Exact match
                    m_match = master_df[master_df["mandal_name"] == m]
                    
                    # 2. Fuzzy match if exact fails
                    if m_match.empty:
                        clean_m = m.replace("(Urban)", "").replace("(Rural)", "").replace(" Municipality", "").replace(" (M)", "").strip()
                        close = difflib.get_close_matches(clean_m, master_mandals, n=1, cutoff=0.85)
                        if close:
                            m_match = master_df[master_df["mandal_name"] == close[0]]
                            
                    if not m_match.empty:
                        lat = float(m_match.iloc[0]["lat"])
                        lon = float(m_match.iloc[0]["long"])
                        final_district = d_name if d_name != "Unknown" else m_match.iloc[0].get("district", "Unknown")
                        ...
                        print(f"    ✓ Local mapped '{m}' -> ({lat:.4f}, {lon:.4f})")
                    else:
                        print(f"    ✗ '{m}' not found in master CSV.")
                        
                if fallback_rows:
                    new_df = pd.DataFrame(fallback_rows)
                    lookup = pd.concat([lookup, new_df], ignore_index=True)
                    lookup.to_csv(save_path, index=False)
                    print(f"\n  ✓ Saved {len(fallback_rows)} mandals from local CSV to {save_path}!")
            except Exception as e:
                print(f"  Failed local CSV mapping: {e}")
    else:
