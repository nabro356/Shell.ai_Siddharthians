...
def load_and_clean(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    name_col = "diagnosis_name" if "diagnosis_name" in df.columns else "diagnosis"
    
    df[code_col] = df[code_col].astype(str).str.strip()
    
    # Track which rows match by code
    mask_code = df[code_col].isin(tracked_codes)
    
    # Track which rows match by keyword
    df_name_lower = df[name_col].astype(str).str.lower()
    mask_kw = pd.Series(False, index=df.index)
    
    # Store exact disease keys for keyword matches
    kw_disease_keys = pd.Series(None, index=df.index, dtype=object)
    
    for dk, info in DISEASE_CODES.items():
        if "keywords" in info:
            for kw in info["keywords"]:
                found = df_name_lower.str.contains(kw.lower(), na=False, regex=False)
                mask_kw = mask_kw | found
                
                # Hierarchical Exclusion: Only assign the disease key if the patient hasn't ALREADY 
                # been assigned to a more specific disease earlier in the dictionary hierarchy.
                # (Specific Pathogens > Broad Syndromes > Generic Fevers)
                to_update = found & kw_disease_keys.isna()
                kw_disease_keys[to_update] = dk

    df_filtered = df[mask_code | mask_kw].copy()
    if verbose:
    ...
    def _map_key(row):
        code_key = code_to_disease(str(row[code_col]).strip())
        if code_key: return code_key
        # Fallback to the keyword mapping we found
        return kw_disease_keys.loc[row.name]

    df_filtered["disease_key"] = df_filtered.apply(_map_key, axis=1)
    df_filtered["disease_name"] = df_filtered["disease_key"].apply(lambda k: DISEASE_CODES[k]["name"] if k else "Unknown")
    if verbose:
