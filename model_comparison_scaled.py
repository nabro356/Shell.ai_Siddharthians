...
def calc_metrics(y_true, y_pred, y_lower=None, y_upper=None):
    ...
    rmse_percent = (rmse / y_mean) * 100 if y_mean > 0 else np.nan
    mae_percent = (mae / y_mean) * 100 if y_mean > 0 else np.nan

    return {
        "rmse_out_of_100": round(rmse_percent, 2) if not np.isnan(rmse_percent) else None,
        "mae_out_of_100": round(mae_percent, 2) if not np.isnan(mae_percent) else None,
    ...
    for fold_idx, train_end in enumerate(split_starts):
        ...
        if models_to_test:
            best_model = min(models_to_test.keys(),
                             key=lambda m: calc_metrics(test_vals, models_to_test[m][0])["rmse_out_of_100"])
            best_rmse = calc_metrics(test_vals, models_to_test[best_model][0])["rmse_out_of_100"]
            print(f"  Fold {fold_idx+1}: Best = {best_model} (RMSE_100={best_rmse})")
...
def summarize_results(all_results):
    ...
    summary = summary.sort_values(["disease", "rmse_out_of_100"]).reset_index(drop=True)
    ...
    for disease in summary["disease"].unique():
        ...
        print(f"\n--- {disease} ---")
        print(d[["model", "rmse_out_of_100", "mae_out_of_100", "mape", "coverage_95", "n_folds"]].to_string(index=False))

        best = d.iloc[0]
        print(f"  → RECOMMENDED: {best['model']} (RMSE_100={best['rmse_out_of_100']:.2f}, MAE_100={best['mae_out_of_100']:.2f})")
