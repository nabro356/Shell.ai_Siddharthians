...
try:
    ...
    HAS_PROPHET = False
    print("WARNING: prophet not installed. Prophet models will be safely skipped. Install: pip install prophet")
...
try:
    ...
    HAS_ML = False
    print("WARNING: xgboost or scikit-learn not installed. Tree models will be safely skipped.")
...
MODELABLE_DISEASES = {
    ...
    "febrile_illness": {
        "min_weeks": 50,
        "covariates": ["mandal_count"],
        "notes": "Very high volume. Needs seasonality adjustments.",
    },
    "typhoid": {
        "min_weeks": 30,
        "covariates": ["mandal_count"],
        "notes": "Enteric fever, moderate volume.",
    },
}
...
def calc_metrics(y_true, y_pred, y_lower=None, y_upper=None):
    ...
    # sMAPE (Symmetric Mean Absolute Percentage Error) bounds 0% to 200%
    denominator = (np.abs(y_true) + np.abs(y_pred)) / 2.0
    nonzero = denominator != 0
    if nonzero.sum() > 0:
        mape = np.mean(np.abs(y_true[nonzero] - y_pred[nonzero]) / denominator[nonzero]) * 100
    else:
        mape = np.nan
...
def fit_xgboost_rf(train_df, horizon, covariates=None, use_rf=False):
    """XGBoost or Random Forest regression with iterative lag forecasting."""
    name = "Random Forest" if use_rf else "XGBoost"
    if not HAS_ML:
        return name, None, None, "Not installed"
    ...
    try:
        ...
        for col in feature_cols:
            if col not in df.columns:
                return name, None, None, f"Missing feature: {col}"
        ...
        predictions = []
        last_known = list(train_df["case_count"].values[-4:])
        last_date = pd.to_datetime(train_df["period"].max())
        ...
        for step in range(horizon):
            ...
            feat_vals = [lag_1, lag_2, lag_4, rolling_4, future_month]
            if covariates:
                for cov in covariates:
                    feat_vals.append(float(train_df[cov].iloc[-1])) 
...
def fit_prophet(train_df, horizon, covariates=None):
    """Bayesian forecasting using Meta's Prophet."""
    name = "Prophet"
    if not HAS_PROPHET:
        return name, None, None, "Not installed"
        
    try:
        df_p = train_df[["period", "case_count"]].copy()
        df_p = df_p.rename(columns={"period": "ds", "case_count": "y"})
        
        if covariates:
            for cov in covariates:
                df_p[cov] = train_df[cov]
        ...
        if covariates:
            for cov in covariates:
                last_cov = train_df[cov].iloc[-1]
                future[cov] = future["ds"].apply(
                    lambda d: train_df.loc[train_df["period"]==d, cov].iloc[0] if d <= train_df["period"].max() else last_cov
                )
        ...
        lower = np.maximum(pred_future["yhat_lower"].values, 0)
        upper = pred_future["yhat_upper"].values
        return name, pred, lower, upper
    except Exception as e:
        return name, None, None, f"Error: {e}"


def generate_time_delay_embedding(df):
    """Prepare time-delay embedding for ML models."""
    df_ml = df.copy()
    for lag in [1, 2, 4]:
        df_ml[f"lag_{lag}"] = df_ml["case_count"].shift(lag)
    df_ml["rolling_4"] = df_ml["case_count"].shift(1).rolling(4, min_periods=1).mean()
    df_ml["month"] = pd.to_datetime(df_ml["period"]).dt.month
    return df_ml
...
def walk_forward_cv(disease_key, ts_weekly, forecast_horizon=4, n_splits=5):
    ...
    disease_data = generate_time_delay_embedding(disease_data)
    
    # Safely impute missing clinical covariate variables (like SpO2) so Prophet/XGBoost don't crash
    cov_cols_global = [c for c in disease_info.get("covariates", []) if c in disease_data.columns]
    if cov_cols_global:
        disease_data[cov_cols_global] = disease_data[cov_cols_global].ffill().bfill().fillna(0)
    ...
    for fold_idx, train_end in enumerate(split_starts):
        ...
        # 3. UCM
        name, pred, lower, upper = fit_ucm(train_vals, actual_horizon, covariates_train=covariates_train, covariates_test=covariates_test)
        if pred is not None:
            models_to_test[name] = (pred, lower, upper)
            
        # 4. NegBin GLM
        name, pred, lower, upper = fit_negbin_glm(train_df, disease_data.iloc[test_start:test_end], target_col="case_count")
        if pred is not None:
            models_to_test[name] = (pred, lower, upper)

        # 5. XGBoost
        name, pred, lower, upper = fit_xgboost_rf(train_df, actual_horizon, covariates=cov_cols, use_rf=False)
        if pred is not None:
            models_to_test[name] = (pred, lower, upper)

        # 6. Random Forest
        name, pred, lower, upper = fit_xgboost_rf(train_df, actual_horizon, covariates=cov_cols, use_rf=True)
        if pred is not None:
            models_to_test[name] = (pred, lower, upper)

        # 7. Prophet
        name, pred, lower, upper = fit_prophet(train_df, actual_horizon, covariates=cov_cols)
        if pred is not None:
            models_to_test[name] = (pred, lower, upper)
        else:
...
def summarize_results(all_results):
    ...
    for disease in summary["disease"].unique():
        ...
        # Format MAPE explicitly as percentage
        d["mape"] = d["mape"].apply(lambda x: f"{min(x, 100.0):.1f}%" if pd.notnull(x) else "N/A")
        
        print(f"\n--- {disease} ---")
        print(d[["model", "rmse", "mae", "mape", "coverage_95", "n_folds"]].to_string(index=False))

        best = d.iloc[0]
        print(f"  → RECOMMENDED: {best['model']} (RMSE={best['rmse']:.2f}, Error Rate={best['mape']})")
...
def plot_final_forecasts(ts_weekly, summary, forecast_horizon=4):
    ...
    for disease_key, disease_info in MODELABLE_DISEASES.items():
        ...
        # Embed Time-Delay array for ML plotting globally
        disease_data = generate_time_delay_embedding(disease_data)
        cov_cols = [c for c in disease_info.get("covariates", []) if c in disease_data.columns]
        if cov_cols:
            disease_data[cov_cols] = disease_data[cov_cols].ffill().bfill().fillna(0)
        ...
        if "Holt" in best_model_name:
            ...
            name, pred, lower, upper = fit_negbin_glm(train_df, train_df, target_col="case_count")
            if pred is not None:
                pred, lower, upper = pred[:forecast_horizon], lower[:forecast_horizon], upper[:forecast_horizon]
        elif "XGBoost" in best_model_name:
            name, pred, lower, upper = fit_xgboost_rf(train_df, forecast_horizon, covariates=cov_cols, use_rf=False)
        elif "Random Forest" in best_model_name:
            name, pred, lower, upper = fit_xgboost_rf(train_df, forecast_horizon, covariates=cov_cols, use_rf=True)
        elif "Prophet" in best_model_name:
            name, pred, lower, upper = fit_prophet(train_df, forecast_horizon, covariates=cov_cols)
        else:
            continue
