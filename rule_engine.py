...
def _eval_threshold_mandal(disease_data, rule, ref_date, ts_mandal_weekly, disease_key):
    """
    Check if any mandal has >= min_cases in the last window_weeks (including forecasts).
    ...
    recent = ts_m[ts_m["period"] >= cutoff]

    if recent.empty:
        return alerts

    mandal_counts = recent.groupby(["mandal", "district"]).agg({"case_count": "sum"}).reset_index()
    triggered = mandal_counts[mandal_counts["case_count"] >= min_cases]
    ...
    for _, row in triggered.iterrows():
        ...
        onset_date = pd.to_datetime(ref_date)
        
        m_hist = ts_m[ts_m["mandal"] == mandal].sort_values("period", ascending=False)
        consecutive_weeks = 0
        
        # Determine the earliest continuous week that satisfies the conditions
        for _, h_row in m_hist.iterrows():
            if h_row["period"] > pd.to_datetime(ref_date):
                if h_row["case_count"] > 0:
...
def evaluate_rules(df_clean, ts_weekly, ts_mandal_weekly=None, ref_date=None):
    ...
    # ---------------------------------------------------------
    # TIME-BOUNDING & SORTING
    # ---------------------------------------------------------
    # The user requested seeing alerts strictly bound between 4 weeks past and 2 weeks future
    alerts_df["onset_date"] = pd.to_datetime(alerts_df["onset_date"])
    ref_date_ts = pd.to_datetime(ref_date)
    
    past_cutoff = ref_date_ts - pd.Timedelta(weeks=4)
    future_cutoff = ref_date_ts + pd.Timedelta(weeks=4)
    
    alerts_df = alerts_df[
        (alerts_df["onset_date"] >= past_cutoff) & 
        (alerts_df["onset_date"] <= future_cutoff)
    ]
    ...
    # Sort primarily by most recent date, then by severity priority
    alerts_df["_priority"] = alerts_df["level"].map(PRIORITY_ORDER)
    alerts_df = alerts_df.sort_values(
        by=["onset_date", "_priority", "cases"], 
